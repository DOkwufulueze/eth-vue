...
module.exports = async deployer => {
  try {
    ...
    response = await deployedDBContract.addPermittedContract(setupAddress, {
      from: coinbase
    });
    console.log(
      "Added Setup Smart Contract to Permitted contracts list: ",
    ...
    response = await deployedDBContract.addPermittedContract(
      userAuthManagerAddress,
      {
        from: coinbase
      }
    );
    console.log(
      "Added UserAuthManager Smart Contract to Permitted contracts list: ",
      isSuccessfulTransaction(response)
    );
  } catch (error) {
    console.error(error);
